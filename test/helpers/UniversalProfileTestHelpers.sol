...
import {LSP1UniversalReceiverDelegateUP as LSP1DelegateUP} from "@lukso/lsp1delegate-contracts/contracts/LSP1UniversalReceiverDelegateUP.sol";
...
// constants
import {_LSP1_UNIVERSAL_RECEIVER_DELEGATE_KEY, _LSP1_UNIVERSAL_RECEIVER_DELEGATE_PREFIX} from "@lukso/lsp1-contracts/contracts/LSP1Constants.sol";
...
import {_TYPEID_LSP7_TOKENSSENDER, _TYPEID_LSP7_TOKENSRECIPIENT} from "@lukso/lsp7-contracts/contracts/LSP7Constants.sol";
...
contract UniversalProfileTestHelpers is Test {
    ...
    function _setUpUniversalProfileLikeBrowserExtension(
        ...
        _setUPMainLSP1DelegateWithPermissions(
            universalProfile,
            mainController,
            mainLSP1DelegateImplementationForUPs
        );
    ...
    function _setUpSpecificLSP1DelegateForTokensSent(
        ...
        bytes32 dataKeyLSP1DelegateForTokensSent = LSP2Utils.generateMappingKey(
            ...
            bytes20(_TYPEID_LSP7_TOKENSSENDER)
        );

        bytes32 dataKeyPermissionsOfLSP1Delegate = LSP2Utils
            .generateMappingWithGroupingKey(
                _LSP6KEY_ADDRESSPERMISSIONS_PERMISSIONS_PREFIX,
                bytes20(abi.encodePacked(specificLSP1Delegate))
            );

        bytes32 lsp1DelegatePermissionsValue = LSP6Utils.combinePermissions(
            lsp1DelegatePermissionsList
        );

        // register the specific LSP1 delegate to react on tokens sent

        universalProfile.setData(
            dataKeyLSP1DelegateForTokensSent,
            abi.encodePacked(specificLSP1Delegate)
        );

        // set the permissions for the specific LSP1 delegate
        universalProfile.setData(
            dataKeyPermissionsOfLSP1Delegate,
            abi.encodePacked(lsp1DelegatePermissionsValue)
        );
    ...
    function _setUpSpecificLSP1DelegateForTokensReceived(
        ...
        bytes32 dataKeyLSP1DelegateForTokensReceived = LSP2Utils
            .generateMappingKey(
                _LSP1_UNIVERSAL_RECEIVER_DELEGATE_PREFIX,
        ...
        universalProfile.setData(
            dataKeyLSP1DelegateForTokensReceived,
            abi.encodePacked(lsp1DelegatePermissionsValue)
        );
    ...
    function _setUPMainLSP1DelegateWithPermissions(
        ...
        universalProfile.setData(
            ...
            abi.encodePacked(mainLSP1DelegateImplementation)
        );

        // give SUPER_SETDATA + REENTRANCY permissions to the main LSP1 Universal Receiver Delegate
        // contract
        bytes32 permissionDataKeyForMainLSP1Delegate = LSP2Utils
            .generateMappingWithGroupingKey(
                _LSP6KEY_ADDRESSPERMISSIONS_PERMISSIONS_PREFIX,
